...
------------------------------------------------------------------------------------------------------------
  http메세지를 직접 parsing하려면 힘들어->
  # HttpsServletRequest
    http요청 정보를 편리하게 사용할수 있도록서블릿이 다 만들어줘서 request객체를 사용하면 됨

  # HttpServeltResponse
   http 응답 정보를 편리하게 사용하도록 제공됨, response객체에 담아서 사용하면 됨

  # response, resquest
  요청 메세지 기반으로 response,request를 새로 객체를 만들어서 파라미터로 넘겨서  servlet을 실행해->response객체안의     찾아서 http응답 메세지 만들고 웹브라우저에 전달

  # 서블릿 컨텐이너 
  서블릿 객체를 직접 생성하는게 아닌 단순 코드만 만들면 was안의 서블릿 컨테이너가 서블릿 객체를 자동으로 생성&호출&생명    주기 관리. 항상 재사용함(싱글톤으로 하나로 만들어놔)
  
  # 동시 요청, 멀티 쓰레드
  쓰레드가 요청이 오면 서블릿객체를 호출

  # 단일 요청, 쓰레드 하나 사용
  요청이 오면 was안에서 쓰레드 할당하고 서블릿 코드 실행->응답-> 쓰레드 휴식
  # 다중 요청, 쓰레드 하나 
  if요청1의 처리가 지연된 상태에서 요청2가 들어오면 쓰레드 대기하면 둘다 오류 가능성 =>요청 마다 쓰레드 생성
  
  # 쓰레드 풀
  쓰레드 풀에 요청이 오면 안에 미리 만들어둔 쓰레드사용하고 사용후 풀에 다시 반납& 대기,거절
  
  # HTTP API
  HTML이 아니라 데이터 전달
  
  # SSR  서버사이드 렌더링
  동적으로 생성된 최종 결과물(html)이 서버에서 생성됨. 
  # CSR 클라이언트 사이드 렌더링
  자바스크립트 사용해서 html생성,바로 html을 받는게 아닌 js에서 
  비어있는 html 내려주고 js를 내리고  js를 요청하고 로직 처리되고, 웹브라우저가 http api로 데이터 요청하면  서버에서     json으로 데이터를 전달.
